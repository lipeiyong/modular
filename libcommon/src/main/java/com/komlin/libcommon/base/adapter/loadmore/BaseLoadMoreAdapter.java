...
public abstract class BaseLoadMoreAdapter<T, V extends ViewDataBinding> extends BaseFooterListAdapter<T, V> {
    ...
    protected <VT extends ViewDataBinding> VT createFooterBinding(ViewGroup parent, int viewType) {
        return DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.comm_list_floor_view, parent, false);
    }

    protected void bindingFooter(ViewDataBinding binding, boolean hasMoreData, Object data) {
        if (binding instanceof CommListFloorViewBinding) {
            if (hasMoreData) {
                ((CommListFloorViewBinding) binding).hint.setText("正在加载更多数据...");
            } else {
                ((CommListFloorViewBinding) binding).hint.setText("没有更多数据！");
            }
        }
    }
